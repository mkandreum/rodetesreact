        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {state.events.filter(e => !e.isArchived).map(event => (
                    <div key={event.id} className="bg-gray-900 border border-gray-800 flex flex-col hover:border-gray-600 transition-all">
                        ...
                        <div className="p-4 flex-grow flex flex-col">
                            ...
                            <p className="text-gray-400 mb-3 whitespace-pre-wrap font-sans text-sm flex-grow">{event.description}</p>
                            <div className="flex justify-between items-center mb-4 pt-3 border-t border-gray-800">
                                <span className="font-pixel text-xl text-party-400">{event.price} €</span>
                                <span className="font-pixel text-gray-500">{new Date(event.date).toLocaleDateString()}</span>
                            </div>
                            {new Date(event.date) > new Date() ? (
                                <button
                                    onClick={() => onSelectEvent(event)}
                                    className="w-full neon-btn font-pixel text-lg py-2"
                                ...
                                <button disabled className="w-full border border-gray-700 text-gray-700 font-pixel text-lg py-2 cursor-not-allowed">EVENTO PASADO</button>
                            )}
                        </div>
                    </div>
                ))}

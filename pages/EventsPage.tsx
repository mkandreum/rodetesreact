                ...
                {state.events.filter(e => !e.isArchived).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map(event => (
                    <div key={event.id} className="relative bg-gray-900 rounded-none overflow-hidden flex flex-col transform transition-all duration-300 reveal-on-scroll visible">
                        ...
                        <div className="p-6 flex flex-col flex-grow">
                            <h3 className={`text-3xl font-pixel ${new Date(event.date) < new Date() ? 'text-gray-500' : 'text-white text-glow-white'} mb-2 cursor-pointer glitch-hover`} onClick={() => onSelectEvent(event)}>
                                {event.name}
                            </h3>
                            <p className="text-gray-400 font-semibold font-pixel text-lg mb-3">{new Date(event.date).toLocaleDateString()}, 21:00</p>
                            <p className={`text-4xl font-extrabold ${new Date(event.date) < new Date() ? 'text-gray-600' : 'text-white'} mb-4`}>{event.price} €</p>

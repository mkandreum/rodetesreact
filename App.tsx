...
import { Calendar, MapPin, Music, Share2, Instagram, Ticket as TicketIcon, Menu, X, Download, Shirt, Edit, Trash, Plus, Image as ImageIcon, CheckCircle, AlertTriangle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleMerchSubmit = (e: React.FormEvent) => {
    ...
    addMerchSale({
      saleId,
      dragId: selectedMerch.drag ? selectedMerch.drag.id : 'web',
    ...
    <div className="min-h-screen bg-[#0f172a] text-white font-sans selection:bg-party-500 selection:text-white">
      ...
      <main className={`container mx-auto px-4 pb-20 pt-32 ${state.promoEnabled ? 'mt-10' : ''}`}>
        ...
        {currentPage === 'home' && (
          <div className="space-y-16">
            ...
            <section>
              <h2 className="text-4xl font-pixel text-white text-center mb-8 text-glow-white">PRÓXIMOS EVENTOS</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                )) : <p className="text-center font-pixel text-gray-500 col-span-full text-2xl">NO HAY EVENTOS PRÓXIMOS</p>}
              </div>
            </section>
          </div>
        )}
        ...
        {currentPage === 'gallery' && (
          <div className="space-y-12">
            ...
            {state.events.filter(e => e.galleryImages.length > 0).length === 0 && (
              ...
              <div className="space-y-8 animate-fade-in">
                <div className="bg-gray-800 p-4 border border-gray-600 flex justify-between items-center rounded-lg shadow-lg">
                   ...
                   <div className="flex gap-3">
                     <button onClick={() => setActiveModal('scanner')} className="bg-indigo-600 text-white font-pixel px-4 py-2 hover:bg-indigo-500 rounded flex items-center gap-2"><Scanner size={16}/> SCANNER</button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-gray-900 border border-white p-6 shadow-[0_0_15px_rgba(255,255,255,0.1)]">
                    <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-2">
                       <h3 className="font-pixel text-2xl">EVENTOS</h3>
                       <button onClick={() => { setEditingEvent({}); setActiveModal('adminEvent'); }} className="text-green-400 hover:text-green-300"><Plus size={24}/></button>
                    </div>
                      ...
                      {state.events.map(e => (
                         <li key={e.id} className="flex justify-between items-center bg-black p-3 border border-gray-800 rounded hover:border-gray-600 transition-colors">
                           <div className="flex-1 min-w-0 pr-4">
                             <p className="font-pixel truncate text-lg">{e.name}</p>
                             <p className="text-xs text-gray-500">
                               Sold: {e.ticketsSold}/{e.ticketCapacity} | {new Date(e.date).toLocaleDateString()}
                             </p>
                           </div>
                           <div className="flex gap-2">
                             <button onClick={() => { setEditingEvent(e); setActiveModal('adminEvent'); }} className="text-blue-400 hover:text-blue-300 p-1"><Edit size={18}/></button>
                             <button onClick={() => { if(confirm('¿Borrar evento?')) deleteEvent(e.id); }} className="text-red-400 hover:text-red-300 p-1"><Trash size={18}/></button>
                           </div>
                         </li>
                      ))}
                    </ul>
                  ...
                  <div className="bg-gray-900 border border-white p-6 shadow-[0_0_15px_rgba(255,255,255,0.1)]">
                    <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-2">
                       <h3 className="font-pixel text-2xl">DRAGS</h3>
                       <button onClick={() => { setEditingDrag({}); setActiveModal('adminDrag'); }} className="text-green-400 hover:text-green-300"><Plus size={24}/></button>
                    </div>
                    <ul className="space-y-3">
                      {state.drags.map(d => (
                         <li key={d.id} className="flex justify-between items-center bg-black p-3 border border-gray-800 rounded hover:border-gray-600 transition-colors">
                           <div className="flex-1 min-w-0 pr-4">
                              <p className="font-pixel truncate text-lg" style={{color: d.cardColor}}>{d.name}</p>
                              <p className="text-xs text-gray-500">Merch Items: {d.merchItems.length}</p>
                           </div>
                           <div className="flex gap-2">
                             <button onClick={() => { setEditingDrag(d); setActiveModal('adminDrag'); }} className="text-blue-400 hover:text-blue-300 p-1"><Edit size={18}/></button>
                             <button onClick={() => { if(confirm('¿Borrar Drag?')) deleteDrag(d.id); }} className="text-red-400 hover:text-red-300 p-1"><Trash size={18}/></button>
                           </div>
                         </li>
                      ))}
                    </ul>
                   ...
                   {/* Quick Stats & Tools */}
                   <div className="bg-gray-900 border border-white p-6 md:col-span-2">
                    <h3 className="font-pixel text-2xl mb-4 border-b border-gray-700 pb-2">HERRAMIENTAS</h3>
                    <div className="flex flex-wrap gap-4">
                       <div className="bg-black border border-gray-800 p-4 rounded flex-1 min-w-[200px]">
                          <p className="text-gray-500 text-sm">TOTAL TICKETS VENDIDOS</p>
                          <p className="text-4xl font-pixel text-party-500">{state.tickets.length}</p>
                       </div>
                       <div className="bg-black border border-gray-800 p-4 rounded flex-1 min-w-[200px]">
                          <p className="text-gray-500 text-sm">TOTAL MERCH PENDIENTE</p>
                          <p className="text-4xl font-pixel text-yellow-500">{state.merchSales.filter(s => s.status === 'Pending').length}</p>
                       </div>
                       <div className="flex-1 min-w-[300px] flex items-end gap-2">
                           <select className="bg-black border border-gray-600 text-white p-2 font-pixel flex-1 h-10 rounded">
                              <option>Sorteo Rápido (Aleatorio)</option>
                           </select>
                           <button 
                             onClick={() => {
                               const winner = state.tickets[Math.floor(Math.random() * state.tickets.length)];
                               alert(winner ? `GANADOR: ${winner.nombre} (${winner.email})` : 'No hay tickets vendidos');
                             }} 
                             className="bg-party-600 text-white font-pixel px-6 h-10 rounded hover:bg-party-500"
                           >
                             EJECUTAR
                           </button>
                       </div>
      ...
      {activeModal === 'scanResult' && scanResult && (
         <div className="fixed inset-0 z-[60] bg-black/95 flex items-center justify-center p-4">
            <div className={`max-w-md w-full p-8 border-4 text-center ${
               scanResult.status === 'success' ? 'border-green-500 bg-green-900/20' : 
      ...
      {activeModal === 'lightbox' && selectedImage && (
         <div className="fixed inset-0 z-50 bg-black/98 flex items-center justify-center p-2" onClick={() => setActiveModal(null)}>
            <button className="absolute top-4 right-4 text-white hover:text-party-500"><X size={40}/></button>
            <img src={selectedImage} className="max-w-full max-h-screen object-contain shadow-2xl" onClick={e => e.stopPropagation()} />
         </div>
      )}
